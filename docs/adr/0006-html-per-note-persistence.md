...
**Status:** Accepted · **Date:** 2026-07-24 · **Updated:** 2026-08-02 · **Supersedes:** the IndexedDB-as-source-of-truth model
...
4. **Body is the sanitized note HTML**, with the allowlist **expanded (minimal)**: add `TABLE, THEAD, TBODY, TR, TH, TD, CAPTION`, `H4, H5, H6`, `HR`, and (2026-08-02) `MARK` for text highlighting. `MARK` carries no attribute, so the highlight survives sanitization without reopening `style` — which the allowlist still strips everywhere.

   **Amendment 2026-08-02 — `class` for syntax highlighting.** Colored code is markup (`<span class="hljs-keyword">`), so `class` is now accepted on `PRE`, `CODE` and `SPAN` only, and only for names matching `hljs`, `hljs-*`, `language-*` or the `name_` submode convention. Every other class name and every other attribute is still stripped. The block's language lives in `class="language-…"` on the `<pre>`, which survives the edit operations that replace the block's children. Rationale: a class name cannot execute, cannot load a remote resource, and the CSP already restricts `style-src`; the alternative (inline `style` on tokens) would have reopened a far larger hole. Consequence: a note file now carries presentation for code, and reading a foreign vault renders unknown token classes as plain uncolored text rather than failing — the color is derived and regenerated on the next edit. A minimal embedded `<style>` in the envelope gives standalone files reasonable rendering. A derived `<aside class="hz-connections">` after the article lists the note's relations for manual reading; it is regenerated on write and is never parsed as note content.

   **Amendment 2026-08-02 — backlinks in the aside.** The aside now also lists the notes that cite this one, with the direction (mutual / outgoing / incoming), the other note's title, an anchor to its file, and both reasons. `hz:connection` still records **only outgoing** edges: a backlink is not a new fact, it is the same edge read from the other side, and storing it twice would let two files disagree with no principled arbiter. Consequence: a note's file can go stale when *another* note changes. Rather than cascading writes into files the user did not edit — which would multiply writes and still fail closed on any externally modified file — the projection is refreshed when the note itself is written, plus an explicit idempotent sweep (`refreshConnectionProjections`, surfaced in the Vault Center) that rewrites only what differs and skips conflicts. Anchors use the indexed physical file name; an external rename breaks a link until the next sweep.
...
- **Backup assistance verifies the selected file.** The UI recommends the first
  export and a new JSON export every seven days. Rust owns the native save
  dialog, writes and synchronizes a temporary file, publishes it, then reopens
  it to confirm byte length and SHA-256 before recording the local timestamp.
  This proves the selected file existed at completion, not that it remains
  durable or off-device afterwards.
