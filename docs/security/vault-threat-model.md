...
| **Denial of service** | HTML ou imagem base64 muito grande consome memória; duplicatas ou SQLite corrompido impedem a carga. | Importação de backup limitada a 120 MB; conflitos são isolados; projeções e embeddings podem ser reconstruídos; operações de escrita no processo são serializadas. | Não há limite por arquivo HTML nem parse incremental. Uma coleção hostil pode causar alta memória/CPU; estado de revisão depende de backup se o SQLite for perdido. |
